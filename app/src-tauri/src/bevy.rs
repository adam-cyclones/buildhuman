...
use crate::mesh_commands::MeshState;
...
pub fn setup_bevy() {
    // Create a Bevy app with default plugins but no window
    let mut app: App = App::new();

    // Use DefaultPlugins but disable the WindowPlugin to prevent creating a separate window
    app.add_plugins(DefaultPlugins.set(WindowPlugin {
        primary_window: None, // Don't create a primary window
        ..default()
    }));

    // Initialize mesh state
    let mesh_state = MeshState::default();
    app.insert_resource(mesh_state.clone());

    // create tauri app
    app.add_plugins(TauriPlugin::new(move || {
        tauri::Builder::default()
            .manage(mesh_state.clone())
            .invoke_handler(tauri::generate_handler![
                greet,
                get_average_frame_rate,
                crate::mesh_commands::generate_base_mesh,
                crate::mesh_commands::save_base_mesh,
                crate::mesh_commands::multi_lerp_meshes,
                crate::mesh_commands::get_base_mesh_count,
                crate::mesh_commands::clear_base_meshes,
                crate::mesh_commands::export_human
            ])
    ...
    // App setup - don't add 3D scene yet, wait for rendering to be ready
