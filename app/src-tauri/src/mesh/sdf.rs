...
pub fn profiled_capsule_sdf(
    ...
    let target_radius = sample_radial_profile(radial_profiles, t, angle);
...
fn sample_radial_profile(profiles: &[Vec<f32>], t: f32, angle: f32) -> f32 {
    ...
    let radius0 = sample_ring_at_angle(&profiles[segment0], normalized_angle);
    let radius1 = sample_ring_at_angle(&profiles[segment1], normalized_angle);
...
fn sample_ring_at_angle(ring: &[f32], angle: f32) -> f32 {
    use std::f32::consts::PI;
